...
Luckily, Java automatically takes care of that for you.
...
public class A_GarbageCollection {
    ...
    Let’s start with the first one. Java provides a method called System.gc().
    Now you might think from the name that this tells Java to run garbage collection.
    Nope! It meekly suggests that now might be a good time for Java to kick off a garbage collection run.
    ...
    Realizing the difference between a "reference" and an "object" goes a long way toward understanding garbage collection, the "new operator",
    and many other facets of the Java language.
    ...
    Line 5 is similar. Draw another box with the string "b" in it this time and an arrow from the word two.
    ...
    On line 7, we have a new variable, so write the word three and draw an arrow from three to "b". Notice that three points to
    what one is pointing to right now and not what it was pointing to at the beginning.
