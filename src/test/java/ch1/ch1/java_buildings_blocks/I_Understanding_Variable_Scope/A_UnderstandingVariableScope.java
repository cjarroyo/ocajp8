...
The variable defined on line 18 is in scope until the block ends on line 23. Using it in the smaller block from lines 19 to 22 is fine.
...
Now that we know where the blocks are, we can look at the scope of each variable. hungry and amountOfFood are method parameters, so they are available for the entire
method. This means their scope is lines 11 to 22. roomInBelly goes into scope on line 12 because that is where it is declared.
...
It goes out of scope on line 20 where the if block ends. amountEaten goes into scope on line 16 where it is declared.
It goes out of scope on line 19 where the while block ends.
...
4:      public void grow(int inches) {  // two local variables
