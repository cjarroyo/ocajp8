...
ods defined in classes, as discussed in Chapter 4. In fact, there is really only one distinction
...
The method getJumpHeight() works just like a static method as defi ned in a class. In other
words, it can be accessed without an instance of the class using the Hop.getJumpHeight() syn-
tax. Also, note that the compiler will automatically insert the access modifier public since all
...
ods are not inherited by a class implementing the interface. The following modifi ed version
...
Contrast this with the behavior you saw for default interface methods in the previous sec-
tion: the code would compile if the subclass overrode the default methods and would fail to
