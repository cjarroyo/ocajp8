...
ment them. That said, like an abstract class, the first concrete class to extend the abstract
...
doesn’t compile.
    ...
    public void classes_Interfaces_and_Keywords() {
        /*
...
Be wary of examples on the exam that mix class and interface definitions. Make sure the
...
syntax.
        ...
        public void Abstract_Methods_and_Multiple_Inheritance(){
...
ible, so you can defi ne a class that fulfills both interfaces simultaneously:
...
and define the “behavior” that the class implementing the interface must have. If two
...
Java to define two methods in a class with the same name and input parameters but differ-
ent return types. Given the following two interface definitions for Herbivore and Omnivore,
...
The code doesn’t compile, as the class defi nes two methods with the same name and
input parameters but different return types. If we were to remove either definition of eat-
Plants(), the compiler would stop because the defi nition of Bear would be missing one
